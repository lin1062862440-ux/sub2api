...
describe('AnnouncementPopup', () => {
  ...
  it('renders the local product-style announcement dialog', async () => {
    const store = useAnnouncementStore()
    store.currentPopup = {
      ...announcement,
      title: 'System maintenance',
      content: 'Save your API key configuration.\n\n- Requests may be delayed',
      status: 'active',
      read_at: null,
    } as any
    ...
    expect(document.body.textContent).toContain('System maintenance')
    expect(document.body.textContent).toContain('Save your API key configuration.')
  ...
  it('keeps the existing user popup dismissal behavior', async () => {
    const store = useAnnouncementStore()
    store.currentPopup = announcement as any
