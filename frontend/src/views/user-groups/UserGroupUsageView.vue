<template>
  <AppLayout>
    <UserGroupDetailShell :group="selectedGroup" :read-only="!canManage">
      ...
      <template v-else>
        <section data-test="usage-filters" class="flex flex-wrap items-end gap-3 border-b border-gray-200 pb-4 dark:border-dark-700">
        ...
        <section v-if="showAdvancedFilters" data-test="advanced-usage-filters" class="grid gap-3 border-b border-gray-200 pb-4 dark:border-dark-700 sm:grid-cols-2">
          <label class="block">
        ...
        <template v-else-if="result">
          ...
          <section v-if="resultView === 'members'" data-test="usage-member-table">
            <div class="overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-900">
              ...
              <template v-else>
                <div class="hidden grid-cols-[minmax(180px,1fr)_110px_140px_140px] gap-4 bg-gray-50 px-5 py-3 text-xs font-medium uppercase text-gray-500 dark:bg-dark-800 dark:text-gray-400 xl:grid">
                  ...
                  <span class="text-right">{{ t('userGroups.usage.teamConsumption') }}</span>
                </div>
                <div
                  v-for="item in result.by_user"
                  :key="item.user_id"
                  class="grid gap-3 border-t border-gray-100 px-5 py-4 first:border-t-0 dark:border-dark-700 xl:grid-cols-[minmax(180px,1fr)_110px_140px_140px] xl:items-center xl:gap-4"
                >
                  <div class="min-w-0">
                  ...
                  <MetricPair :label="t('userGroups.usage.teamConsumption')" :value="formatCurrency(item.total_actual_cost)" />
                </div>
              </template>
            </div>
          </section>
