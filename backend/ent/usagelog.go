...
type UsageLog struct {
	...
	// 最终按团队订阅结算时写入的业务用户组快照
	BusinessUserGroupID *int64 `json:"business_user_group_id,omitempty"`
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldBusinessUserGroupID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field business_user_group_id", values[i])
			} else if value.Valid {
				_m.BusinessUserGroupID = new(int64)
				*_m.BusinessUserGroupID = value.Int64
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.BusinessUserGroupID; v != nil {
		builder.WriteString("business_user_group_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
