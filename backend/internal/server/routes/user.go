...
func registerUserGroupRoutes(authenticated *gin.RouterGroup, h *handler.UserGroupHandler, heavy gin.HandlerFunc) {
	groups := authenticated.Group("/user-groups")
	{
		...
		groups.PUT("/:id/team-subscription-groups", h.ReplaceTeamSubscriptionGroups)
		groups.GET("/:id/subscriptions", heavy, h.ListSubscriptions)
		groups.GET("/:id/usage", heavy, h.GetUsage)
		groups.GET("/:id/quota", h.GetQuotaOverview)
		groups.PUT("/:id/quota-policy", h.SetQuotaPolicy)
		groups.PUT("/:id/quota-managers", h.ReplaceQuotaManagers)
		groups.PUT("/:id/member-quotas", h.UpdateMemberQuotas)
		groups.POST("/:id/quota-reset", h.ResetQuotaUsage)
