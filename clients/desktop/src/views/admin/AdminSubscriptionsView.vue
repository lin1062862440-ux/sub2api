...
import type { AdminGroupOption, AdminSubscription, AdminSubscriptionProgress } from '@/api/admin/types'
...
const exhausted = computed(() => items.value.filter((item) => {
  ...
  return [windows?.daily, windows?.weekly, windows?.monthly].some(window => window && window.limit != null && window.percentage >= 100)
}).length)
  ...
  <div class="page">
    ...
    <section v-else class="list">
      <article v-for="item in items" :key="item.id">
        ...
        <div class="windows">
          <div v-for="window in ['daily', 'weekly', 'monthly'] as const" :key="window"><div><span>{{ window === 'daily' ? '日额度' : window === 'weekly' ? '周额度' : '月额度' }}</span><strong>{{ progress[item.id]?.[window]?.percentage ?? 0 }}%</strong></div><i><b :class="{ full: (progress[item.id]?.[window]?.percentage ?? 0) >= 100 }" :style="{ width: `${percentage(progress[item.id]?.[window]?.percentage)}%` }" /></i><small>{{ formatCost(progress[item.id]?.[window]?.used) }} / {{ progress[item.id]?.[window]?.limit == null ? '不限' : formatCost(progress[item.id]?.[window]?.limit) }}</small></div>
        </div>
