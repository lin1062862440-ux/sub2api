      ...
      <div v-if="modelValue" class="ug-backdrop" data-testid="user-group-editor" @mousedown.self="close">
        <section class="ug-dialog ug-dialog-compact" role="dialog" aria-modal="true" aria-labelledby="user-group-dialog-title">
          <header class="ug-dialog-head">
            ...
            <div><h2 id="user-group-dialog-title">{{ group ? '编辑团队' : '新建团队' }}</h2><p>{{ group ? '更新组织名称和说明' : '创建后可以继续添加成员与查看者' }}</p></div>
          ...
          <form class="ug-form" data-testid="user-group-editor-form" @submit.prevent="submit">
            <label><span>团队名称</span><input v-model="form.name" data-testid="user-group-name" maxlength="100" placeholder="例如：研发团队" autofocus /></label>
            <label><span>说明</span><textarea v-model="form.description" data-testid="user-group-description" rows="3" placeholder="说明这个团队的用途" /></label>
            <p class="ug-info">团队只负责组织人员和汇总数据。计费倍率、平台类型与额度限制仍由“分组管理”维护。</p>
            <p v-if="error" class="ug-error" role="alert">{{ error }}</p>
            <footer><button type="button" class="secondary" @click="close">取消</button><button type="submit" :disabled="saving || !form.name.trim()"><LoaderCircle v-if="saving" :size="16" class="spinning" /><Save v-else :size="16" />{{ saving ? '保存中' : group ? '保存变更' : '创建团队' }}</button></footer>
          </form>
        </section>
      </div>
    </Transition>
