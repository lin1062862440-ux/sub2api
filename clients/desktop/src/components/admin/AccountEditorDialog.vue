...
async function submit() {
  ...
  try {
    ...
    error.value = caught instanceof Error && caught.message ? caught.message : '账号保存失败'
  } finally {
    saving.value = false
  }
}
...
<template>
  <Transition name="dialog-fade">
    <div v-if="modelValue" class="dialog-backdrop" @mousedown.self="close">
      <section class="account-editor" role="dialog" aria-modal="true" aria-labelledby="account-editor-title">
        <header>
          <span><ServerCog :size="20" /></span>
          <div><h2 id="account-editor-title">{{ editing ? '编辑账号' : '新增账号' }}</h2><p>维护常用接入参数与调度容量</p></div>
          <button type="button" title="关闭" aria-label="关闭" @click="close"><X :size="18" /></button>
        </header>
        ...
        <form data-testid="account-editor-submit" @submit.prevent="submit">
          ...
          <footer class="wide"><button type="button" class="secondary" @click="close">取消</button><button type="submit" :disabled="saving"><LoaderCircle v-if="saving" :size="16" class="spinning" /><Save v-else :size="16" />{{ saving ? '保存中' : '保存账号' }}</button></footer>
        </form>
      </section>
    </div>
  </Transition>
