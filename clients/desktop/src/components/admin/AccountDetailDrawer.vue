...
defineProps<{ account: AdminAccount | null }>()
...
<template>
  <Transition name="drawer">
    <div v-if="account" class="drawer-backdrop" @mousedown.self="emit('close')">
      <aside class="account-detail" data-testid="account-detail">
        ...
        <section v-if="account.error_message" class="error-box"><span>最近错误</span><p>{{ account.error_message }}</p></section>
        <section class="detail-grid"><div><span>当前并发</span><strong>{{ account.current_concurrency ?? 0 }} / {{ account.concurrency }}</strong></div><div><span>优先级</span><strong>{{ account.priority }}</strong></div><div><span>计费倍率</span><strong>{{ account.rate_multiplier ?? 1 }}x</strong></div><div><span>代理</span><strong>{{ account.proxy_id ? `#${account.proxy_id}` : '直连' }}</strong></div></section>
