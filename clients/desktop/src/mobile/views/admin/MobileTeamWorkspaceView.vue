...
<template>
  ...
  >
    ...
    <section v-if="overview" class="mobile-quota-summary" data-testid="mobile-team-quota-summary">
      ...
      <div><span>本周用量</span><strong>{{ formatCost(weeklyUsed) }}</strong></div>
    ...
    <section class="mobile-member-list">
      ...
      <article v-for="member in members" :key="member.user_id" class="mobile-member-row" :data-testid="`mobile-team-member-${member.user_id}`">
        ...
        <div class="mobile-member-metrics"><span><small>本周用量</small><strong>{{ formatCost(quotaMember(member.user_id)?.weekly_usage_usd ?? 0) }}</strong></span><span><small>使用进度</small><strong>{{ usagePercent(member.user_id) }}%</strong></span></div>
