...
async function loadUsers(
  ...
  try {
    ...
    if (requestedPage > availablePages) {
      await loadUsers(availablePages, true, feedbackToken, reportFailure)
      return
    }
...
async function loadGroups() {
  ...
  try {
    ...
    if (mounted && generation === groupGeneration) groups.value = []
  }
}
...
async function openDetail(user: AdminUser) {
  ...
  try {
    ...
    if (mounted && detailUser.value?.id === user.id && response?.id === user.id) detailUser.value = response
  } catch {
    if (mounted && detailUser.value?.id === user.id) {
      const token = claimFeedback()
...
function requestStatusChange(user: AdminUser) {
  if (pendingByUser[user.id]) return
  focusMenuTrigger(user.id)
...
<template>
  <MobilePage
    ...
    :aria-busy="busy"
    loading-label="正在加载用户"
    empty-title="暂无用户"
    empty-message="当前筛选范围内没有用户。"
    @retry="retryUsers"
    @refresh="retryUsers"
  >
    <template #action>
    ...
    <div class="users-content">
      ...
      <p v-if="actionError" class="action-error" data-testid="user-action-error" role="alert"><AlertCircle :size="17" />{{ actionError }}</p>
      <div v-if="listLoading" class="list-busy" role="status">正在刷新用户</div>

      <section class="user-list" aria-label="用户列表">
        <article v-for="user in result.items" :key="user.id" class="user-card" data-testid="mobile-user-card">
          <header>
            ...
            <div class="identity"><strong>{{ safeText(user.username, '未命名用户') }}</strong><span>{{ safeText(user.email, '未提供邮箱') }}</span></div>
            <div class="menu-owner">
              ...
              <div v-if="openMenuId === user.id" class="user-menu" :data-testid="`user-menu-${user.id}`" role="menu">
                ...
                <button type="button" role="menuitem" :data-testid="`toggle-user-${user.id}`" :disabled="Boolean(pendingByUser[user.id])" @click="requestStatusChange(user)"><Power :size="17" />{{ user.status === 'active' ? '停用用户' : '启用用户' }}</button>
                <button class="danger" type="button" role="menuitem" :data-testid="`delete-user-${user.id}`" :disabled="Boolean(pendingByUser[user.id])" @click="openDelete(user)"><Trash2 :size="17" />删除用户</button>
              </div>
            </div>
          </header>

          <div class="status-row"><span class="role" :class="user.role">{{ roleLabel(user.role) }}</span><span class="status" :class="user.status">{{ statusLabel(user.status) }}</span></div>
          <dl>
            <div><dt>可用余额</dt><dd :data-testid="`user-balance-${user.id}`">{{ safeMoney(user.balance) }}</dd></div>
            <div><dt>当前 / 上限并发</dt><dd :data-testid="`user-concurrency-${user.id}`">{{ concurrencyLabel(user) }}</dd></div>
            <div><dt>专属分组</dt><dd :data-testid="`user-group-count-${user.id}`">{{ groupCount(user) }}</dd></div>
          </dl>
          <p class="last-active">最近活跃 {{ safeDate(user.last_active_at ?? user.last_used_at) }}</p>
    ...
    <UserGroupsDialog :user="groupsUser" :groups="groups" mobile @close="groupsUser = null" @updated="handleGroupsUpdated" />
    <UserDetailDrawer :user="detailUser" mobile @close="detailUser = null" @updated="replaceUser" />
