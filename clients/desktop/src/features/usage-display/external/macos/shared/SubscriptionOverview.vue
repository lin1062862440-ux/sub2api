    ...
    <div v-if="subscription && quotaSummary" class="subscription-content">
      <div class="subscription-primary">
        ...
        <div class="subscription-copy">
          ...
          <span><CalendarClock :size="13" />{{ dateLabel(subscription.expires_at) }} 到期</span>
        </div>
      </div>
      <div v-if="quotaSummary.quotas.length" class="quota-list">
        <QuotaRow v-for="quota in quotaSummary.quotas" :key="quota.key" :quota="quota" />
      </div>
      <div v-else class="unlimited-state"><CircleGauge :size="17" /><span>当前订阅无周期额度限制</span></div>
    </div>
